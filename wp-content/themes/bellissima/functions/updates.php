...
function update_notifier_menu() {  
	...
	if(version_compare($theme_data['Version'], $xml->latest) == -1) {
		add_dashboard_page( $theme_data['Name'] . 'Theme Updates', $theme_data['Name'] . '<span class="update-plugins count-1"><span class="update-count">Updates</span></span>', 'administrator', strtolower($theme_data['Name']) . '-updates', update_notifier);
	}
}  
...
function get_latest_theme_version($interval) {
	...
	$notifier_file_url = 'http://unifato.net/bellissima/updates.xml';
