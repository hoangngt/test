...
;(function($) {
	...
	*/
	var Image = function(src, thumb, caption, data, fullimage) {
		this.src = src;
		this.thumb = thumb;
	...
	$.fn.exposure = function(options) {
		...
		$.extend(this, {
			...
			*/
			newImage : function(src, thumb, caption, data, fullimage) {
				var alreadyAdded = ex.isDefined(gallery.sources[src]);
				if (alreadyAdded && !gallery.allowDuplicates) {
					return -1;
				}
				var image = new Image(src, thumb, caption, data, fullimage);
			...
			* @param data Extra image data.
			*/		
			addImage : function(src, thumb, caption, data, fullimage) {
				var pageCount = gallery.numberOfPages();		
				var index = gallery.newImage(src, thumb, caption, data, fullimage);
				if (index > -1) {
						...
						if (src) {
							...
							var img = gallery.loadImage(index, function() {
								...
								if ($(".zoom").length > 0){
									...
									wrapper.append($(this));
									$(this).wrap('<a href="'+fullimage+'" class="single_product_image zoom"></a>');
								}
			...
			if ($(this).children('li').length) {
				...
				$(this).show().children('li').each(function() {
					...
					if (a.length) {
						...
						var imageIndex = gallery.newImage(src, thumbSrc, caption, thumbData, fullimage);
...
jQuery(document).ready(function($) {
	...
	gallery.exposure({controlsTarget : '#controls',
		...
		onImage : function(image, imageData, thumb, fullimage) {
			$('a.single_product_image').fancybox();
