...
<?php if ( $woocommerce->cart->ship_to_billing_address_only() && $woocommerce->cart->needs_shipping() ) : ?>
	...
	<h4 class="related_products_title"><?php _e('Billing &amp; Shipping', 'woocommerce'); ?></h4>
	
<?php else : ?>

	<h4 class="related_products_title"><?php _e('Billing Address', 'woocommerce'); ?></h4>
...
<?php if (!is_user_logged_in() && get_option('woocommerce_enable_signup_and_login_from_checkout')=="yes") : ?>
	...
	<?php if (get_option('woocommerce_enable_guest_checkout')=='yes') : ?>
		...
		<p class="form-row">
			...
			<div class="woocommerce_info"><label for="createaccount" class="checkbox"><?php _e('Create an account?', 'woocommerce'); ?></label></div>
		</p>
